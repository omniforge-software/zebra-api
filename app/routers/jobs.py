...
from app.models.schemas import DirectPrintRequest, PrintJobOut, PrintRequest
from app.services.jobs import DIRECT_ZPL_TEMPLATE_ID, create_job, process_print_job
...
@router.post("/print/zpl", response_model=PrintJobOut, status_code=202)
async def submit_zpl(request: DirectPrintRequest, api_key: ApiKey = Depends(require_api_key)):
    """Submit one fully rendered label format without a saved template."""
    try:
        job = create_job(
            request.printer_id, DIRECT_ZPL_TEMPLATE_ID,
            {"zpl": request.zpl}, request.quantity, api_key.id,
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    asyncio.create_task(process_print_job(job.id))
